(function ($) {
	...
	function getChildBodyDom(rb, v, cd,data_num) {

        var childJsonData = getJsonData(v.child_field, cd);
		
		var formatter=v.formatter;
		if(typeof childJsonData!='undefined' && childJsonData!=null && childJsonData.length>0){
			rb.push("<tr class='sgrid-tr sgrid-tr-body'><td colspan='"+data_num+"'><table>");
			$.each(childJsonData,function(k,item){
				if(typeof v.child_array!='undefined' && v.child_array!=null && v.child_array.split(",").length>0){
					rb.push("<tr><td>");
					var item_formatter=formatter;
					$.each(v.child_array.split(","),function(kk,item_name){
						
						item_formatter=item_formatter.replace(item_name,item[item_name]);
						/*if(item_name.indexOf("jyimg_")==0){ item_formatter.replace(item_name,item[item_name]);
							console.log(item_name.substr(6));
							rb.push("<img style='width:66px;' src='"+item[item_name.substr(6)]+"'/>");
						}else{
							rb.push(item[item_name]+"&nbsp;");
						} */
						
					});
					rb.push(item_formatter);
					rb.push("</td></tr>");
				}
			})
			rb.push("</table><td></tr>");
    ...
    function getHeaderEl(t) {
        ...
        $.each(columns, function (k, v) {
            if (v.field == "checkbox" && data.options.isMultiSelected == true) {
                rh.push("<th width='"+ v.width +"' style='text-align: center'><input type='checkbox' name='"+v.name+"' value='0' onchange='"+v.event+"' /></th>");
            }else if(v.field == "#"){
                rh.push('<th width="'+ v.width +'" style="text-align: center">序号</th>')
            }else if(v.field == "tr"){
    ...
    function renderGrid(t, d, hasHeader) {
        ...
        $.each(d, function (i, j) {
            ...
            $.each(columns, function (k, v) {
                try{
                    if (v.field == "checkbox") {
                    	rb.push("<td style='text-align: center'><input type='checkbox' name='"+v.name+"' value='"+j[v.value]+"' /></td>");
                    }
                    else if (v.field == "#"){
                        rb.push('<td style="text-align: center">'+(i + 1)+'</td>');
                    }else if(v.field == "tr"){
						getChildBodyDom(rb,v, j,columns.length-1);
                    }else {
                        var multirow = v.multirow;
                        if (multirow) {
                            var jd = j[v.field];
